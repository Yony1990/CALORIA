...
const defaultProfile = {
  ...
  dietType: 'balanced',
  targetWeightLoss: 0.5, // kg per week
};

const defaultTracking = {
  water: 0, // glasses (250ml each)
  waterGoal: 8,
  currentWeight: 70,
  bodyFat: null,
  waist: null,
...
export function useAppState() {
  ...
  const [profile, setProfile] = useState(() => {
    ...
    // Primera vez — guardar peso inicial
  ...
  const saveProfile = useCallback((data) => {
    ...
    // Guardar peso inicial solo si no existe todavía
  ...
  const getTotalConsumed = useCallback(() => {
    ...
    return {
      cal: Math.round(allMeals.reduce((s, i) => s + (i.cal * i.grams / 100), 0)),
      protein: Math.round(allMeals.reduce((s, i) => s + (i.protein * i.grams / 100), 0)),
      carbs: Math.round(allMeals.reduce((s, i) => s + (i.carbs * i.grams / 100), 0)),
      fat: Math.round(allMeals.reduce((s, i) => s + (i.fat * i.grams / 100), 0)),
      fiber: Math.round(allMeals.reduce((s, i) => s + ((i.fiber || 0) * i.grams / 100), 0)),
    };
  }, [meals]);
  ...
  const totals = allMeals.reduce((acc, item) => {
    ...
    return {
      vitC:      acc.vitC      + ((item.vitC      || 0) * ratio),
      vitD:      acc.vitD      + ((item.vitD      || 0) * ratio),
      vitB12:    acc.vitB12    + ((item.vitB12    || 0) * ratio),
      iron:      acc.iron      + ((item.iron      || 0) * ratio),
      calcium:   acc.calcium   + ((item.calcium   || 0) * ratio),
      magnesium: acc.magnesium + ((item.magnesium || 0) * ratio),
      zinc:      acc.zinc      + ((item.zinc      || 0) * ratio),
      potassium: acc.potassium + ((item.potassium || 0) * ratio),
      omega3:    acc.omega3    + ((item.omega3    || 0) * ratio),
      fiber:     acc.fiber     + ((item.fiber     || 0) * ratio),
    };
  }, { vitC:0, vitD:0, vitB12:0, iron:0, calcium:0, magnesium:0, zinc:0, potassium:0, omega3:0, fiber:0 });

  // Metas diarias recomendadas
  const goals = {
